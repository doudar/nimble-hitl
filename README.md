...
- `.github\workflows\windows-installer.yml` defines the Windows release pipeline for GitHub-hosted builds, runs on manual dispatch or `v*` tag pushes, and publishes the generated installer as a GitHub release asset.
- Manual runs derive the release tag from `host_app\pubspec.yaml` as `v<version>`, create that tag if it does not already exist, and refuse to move an existing version tag to a different commit.
